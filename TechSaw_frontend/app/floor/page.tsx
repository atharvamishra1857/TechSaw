...
const BLOCKER_REASONS = process.env.NEXT_PUBLIC_BLOCKER_REASONS?.split(",") || ["Stopped"];
...
export default function FloorKiosk() {
  ...
  const fetchData = async () => {
    try {
      const [pulseRes, drawRes] = await Promise.all([
        ...
        fetch("http://127.0.0.1:8000/api/floor/drawings")
      ]);
  ...
  const handleResume = async (id: string) => {
    await fetch(`http://127.0.0.1:8000/api/pulse/${id}/resume`, { method: "POST" });
    fetchData();
  };
  ...
  const handleAssignJob = async (pulseId: string, inquiryId: string) => {
    ...
    await fetch(`http://127.0.0.1:8000/api/pulse/${pulseId}/assign/${inquiryId}`, { method: "POST" });
    fetchData();
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pulses.map((pulse) => {
            ...
            return (
              <div key={pulse.id} className={`flex flex-col justify-between rounded-2xl p-6 border-4 transition-all min-h-[320px] ${isStopped ? "bg-red-950/30 border-red-600" : "bg-slate-800 border-slate-700"}`}>
                
                {/* 1. Header & Current Job */}
                <div className="mb-6">
                  <div className="flex justify-between items-start">
                    <h2 className="text-2xl font-black mb-1">{pulse.machine.name}</h2>
                ...
                <div className="mt-auto">
                      ...
                      <div className="max-h-48 overflow-y-auto flex flex-col gap-2 pr-1 custom-scrollbar">
                        {drawings.length === 0 ? (
                          ...
                          drawings.map(draw => (
                            ...
                            >
                              <div className="font-bold text-white group-hover:text-white">{draw.client_name}</div>
                              <div className="text-xs text-slate-400 group-hover:text-indigo-100 mt-1">{draw.job_dimension} · {draw.job_material}</div>
                            </button>
                          ))
                        )}
                      </div>
                  ...
                  {!isStopped && !isHalting && !isAssigning && (
                    <div className="flex gap-2">
                      ...
                      <button 
                        onClick={async () => {
                          if(window.confirm("Are you sure this job is 100% complete?")) {
                            await fetch(`http://127.0.0.1:8000/api/pulse/${pulse.id}/complete/${pulse.order.id}`, { method: "POST" });
                            fetchData();
                          }
                        }}
